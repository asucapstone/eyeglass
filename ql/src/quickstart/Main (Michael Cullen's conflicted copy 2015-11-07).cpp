...
#define OUTPUT_PATH	"C:\Users\Michael\Desktop\QuickLINK_2_2.7.3.2\bin\output.txt"
#define CALIBRATION_PATH "C:\Users\Michael\Desktop\QuickLINK_2_2.7.3.2\bin\calibration"
			...
			if (QLCalibration_Load(calibrationLocation, &calibrationId) == QL_ERROR_OK) 
			{
				...
				while ((_kbhit() == 0) || (_getch() != 27))
				{
					...
					if (frameData.WeightedGazePoint.Valid) {
						...
						fprintf_s(
							output,
							"\r\"Frame\": %-10d,\"X\":%6.2f%%,\"Y\":%6.2f%%,\"V\":%d,\"Timestamp\": %6.2f",
							frameData.ImageData.FrameNumber,
							frameData.WeightedGazePoint.x,
							frameData.WeightedGazePoint.y,
							1,
